    ...
    def play(self, start_name, goal_name):
        ...
        while current != goal:
            ...
            for i, (door, (neighbor, cost)) in enumerate(current.doors.items()):
                print(f" {i+1}. Door '{door}' → {neighbor.name} | Hint: {current.hints[door]}")

            choice = input("Choose a door number: ")
        ...
        string_BFS = " → ".join([n.name for n in shortest_found_by_BFS])

        print(f"\n🎉 You reached the goal {goal.name} with total steps taken {steps_taken}!")
